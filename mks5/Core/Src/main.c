...
static void uart_process_command(char *cmd)
{
	...
	else if (stscasecmp(token, "DUMP") == 0) {
		uint8_t mem = 0;
		for(int i_add = 0; i <= 16, i++) {

			HAL_I2C_Mem_Read(&hi2c1, EEPROM_ADDR, i_add, I2C_MEMADD_SIZE_16BIT, &mem, 1, 1000);
			printf("%d ", mem);
		}
	}
	/*
	else if (strtok(token, "DUMP"))
	*/
	else goto error;
